...
//TIGER060 重要 media 媒体协商和处理，原来在mod_sofia模块，为了支持webrtc放这里
		...
		if (type != SWITCH_MEDIA_TYPE_TEXT && engine->reset_codec > 0) {
			...
			if (switch_rtp_ready(engine->rtp_session)) {
				...
				if (type == SWITCH_MEDIA_TYPE_AUDIO && engine->read_impl.samples_per_second) {
					if ((val = switch_channel_get_variable(session->channel, "rtp_timeout_sec"))) {//tiger 用的最多的超时
						int v = atoi(val);
						if (v >= 0) {
							rtp_timeout_sec = v;
						}
...
//tiger SDP 协商重要，Mod_verto (verto_tech_media, verto__attach_func)和sofia(sofia_media_negotiate_sdp)同时调用这个函数
...
//tiger rtp 重要 是sofia 里面抽取出来给verto和sofia用, 
...
SWITCH_DECLARE(void) switch_core_media_gen_local_sdp(switch_core_session_t *session, switch_sdp_type_t sdp_type, const char *ip, switch_port_t port, const char *sr, int force)
{
	...
	const char *append_audio = switch_channel_get_variable(session->channel, "rtp_append_audio_sdp");//TIGER APPEND SDP
...
//TIGER1 写的时候增加一个media_bug， 可以在发送之前对数据进行修改和替换
	if (session->bugs) {
		switch_media_bug_t *bp;
		int prune = 0;
